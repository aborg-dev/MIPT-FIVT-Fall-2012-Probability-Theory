...
\begin{theorem}[Александров]~
  ...
  \begin{enumerate}
    \item $P_n \toup{w} P$
    \item $\limup_{n \to \infty} P_n (A) \leq P(A), \quad \forall \text{ замкнутого } A$
    \item $\limdown_{n \to \infty} P_n (A) \geq P(A), \quad \forall \text{ открытого } A$
    \item $P_n \implies P$
  \end{enumerate}
\end{theorem}

\begin{theorem}[Эквивалентность определений сходимости]~
...
Это аппроксимация распределений.
